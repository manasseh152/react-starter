    ...
    // eslint-disable-next-line unused-imports/no-unused-vars
      ...
      if (/^([01]?\d|2[0-3]):[0-5]\d(:[0-5]\d)?$/.test(timeValue)) {
        ...
        return Temporal.PlainTime.from({
          ...
          second: parts.length > 2 ? Number.parseInt(parts[2], 10) : 0,
          millisecond: 0,
        });
      }
...
// // TODO: Uncomment this function if you want to support HH:MM time format
//
    ...
    if (isValidISODate(inputValue)) {
      try {
        ...
        if (min !== undefined) {
          ...
          if (Temporal.PlainDate.compare(plainDate, minPlainDate) < 0)
            return;
        }

        if (max !== undefined) {
          const maxPlainDate = toPlainDate(max);
          if (Temporal.PlainDate.compare(plainDate, maxPlainDate) > 0)
            return;
        }
